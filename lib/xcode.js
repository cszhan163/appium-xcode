...
async function runXcrunCommand (args, timeout = XCRUN_TIMEOUT, opts=null) {
  try {
    let cmdStr = XCODE_CRUN
    if (opts && opts.xcodepath){

        let cmdPath = opts.xcodepath+XCODE_CRUN;

         if(fs.exists(cmdPath)){

            cmdStr = cmdPath;
            log.debug('with xcrun: '+ cmdStr);
         }

    }
        
    return await exec(cmdStr, args, {timeout});
  } catch (err) {
...
async function getPathFromXcodeSelect (timeout = XCRUN_TIMEOUT, opts=null) {
  
  // trim and remove trailing slash
  let  xcodeFolderPath = null
  if(opts.xcodepath == null) {
    log.debug("not have xcodepath")
    let {stdout} = await exec('xcode-select', ['--print-path'], {timeout});
    xcodeFolderPath = stdout.replace(/\/$/, '').trim();
  } else {
    let stdout = opts.xcodepath
    xcodeFolderPath = stdout.replace(/\/$/, '').trim();
  }

  log.debug("path of xcodepath: "+xcodeFolderPath)
...
const getPath = _.memoize(function (timeout = XCRUN_TIMEOUT, opts=null) {
  // first we try using xcode-select to find the path
  // then we try using the symlinks that Apple has by default
  return getPathFromXcodeSelect(timeout,opts).catch(getPathFromSymlink);
});



async function getVersionWithoutRetry (timeout = XCRUN_TIMEOUT, opts=null) {
  let xcodePath = await getPath(timeout,opts);
   log.debug("get xcode path: "+ xcodePath)
  // we want to read the CFBundleShortVersionString from Xcode's plist.
  // It should be in /[root]/XCode.app/Contents/
  const plistPath = path.resolve(xcodePath, "..", "Info.plist");
   log.debug("get version path: "+ plistPath)
...
const getVersionMemoized = _.memoize(
  function (retries = DEFAULT_NUMBER_OF_RETRIES, opts=null, timeout = XCRUN_TIMEOUT) {
    return retry(retries, getVersionWithoutRetry, timeout,opts);
  }
);

async function getVersion (parse = false, opts = null, retries = DEFAULT_NUMBER_OF_RETRIES, timeout = XCRUN_TIMEOUT) {
  //log.debug("times: "+String(retries)+"opts: "+opts)
  const version = await getVersionMemoized(retries, opts,timeout);
...
async function getMaxIOSSDKWithoutRetry (timeout = XCRUN_TIMEOUT, opts = null) {
  const version = await getVersion(false, opts,DEFAULT_NUMBER_OF_RETRIES, timeout);
  if (version[0] === '4') {
    return '6.1';
  }

  const args = ['--sdk', 'iphonesimulator', '--show-sdk-version'];
  const {stdout} = await runXcrunCommand(args, timeout,opts);
...
const getMaxIOSSDK = _.memoize(
  function (opts=null, retries = DEFAULT_NUMBER_OF_RETRIES, timeout = XCRUN_TIMEOUT) {
    return retry(retries, getMaxIOSSDKWithoutRetry, timeout,opts);
  }
);
...
async function getInstrumentsPathWithoutRetry (timeout = XCRUN_TIMEOUT, opts = null) {
  const args = ['-find', 'instruments'];
  let {stdout} = await runXcrunCommand(args, timeout, opts);
...
const getInstrumentsPath = _.memoize(
  function (opts = null, retries = DEFAULT_NUMBER_OF_RETRIES, timeout = XCRUN_TIMEOUT) {
    return retry(retries, getInstrumentsPathWithoutRetry, timeout,opts);
  }
);
